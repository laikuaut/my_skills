...
description: |
  React/Next.jsの実装コードから画面仕様書（Markdown）をリバースエンジニアリングで自動生成するスキル。画面ごとにUI要素・アクション・遷移・API通信・ユースケースを網羅的に記載し、画面遷移図（Mermaid）と画面一覧をまとめたindex.mdも作成する。
  使用するケース: 「画面仕様書を作って」「仕様書をリバースで」「既存画面のドキュメントを起こして」「画面一覧を作って」「コードから仕様書」「UIドキュメント」「画面設計書」など、既存実装からの仕様書化依頼。
  使わないケース: 新規画面の設計（まだコードがない場合）、Vue/Svelte等の他フレームワーク、APIドキュメント生成（openapi-docsを使う）。
  オプション: Playwrightでの画面キャプチャ＋UI要素IDアノテーションは「キャプチャ付きで」と指示された場合のみ実行（詳細はStep 4）。
---
...
4. **画面キャプチャ（オプション）** — ユーザが「キャプチャ付きで」と指示した場合のみ、Playwrightで各画面状態のスクリーンショットを撮影し、UI要素IDのアノテーションバッジを付与する。指示がなければスキップ
...
## Step 4: 画面キャプチャ（アノテーション付き、オプション）

> **このステップはオプション**: ユーザが「キャプチャ付きで」「スクリーンショット込みで」と明示的に指示した場合のみ実行する。指示がなければスキップしてStep 5へ進む。Playwrightのセットアップやサーバ起動が必要なので、軽量な仕様書化だけが目的のときは不要。
