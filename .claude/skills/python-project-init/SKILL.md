...
description: |
  Pythonプロジェクトの初期構築スキル。uv・Docker・ruff・pytestを含むプロジェクト雛形を生成する。
  ライブラリ・バックエンド（FastAPI）・コマンド（CLI）の3構成に対応。
  「Pythonプロジェクトを作って」「新規プロジェクト」「雛形を作って」「スケルトン作成」
  「uvで初期化」「FastAPIプロジェクト」「CLIツールを作りたい」「ライブラリを作りたい」
  などプロジェクトの新規作成を求められたときに使用する。既存コードの編集には使わない。
---
